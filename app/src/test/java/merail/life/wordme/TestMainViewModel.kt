...
class MainViewModelTest {
    ...
    private val configRepository: IConfigRepository = mockk(relaxed = true)
    private val databaseRepository: IDatabaseRepository = mockk(relaxed = true)
    private val storeRepository: IStoreRepository = mockk(relaxed = true)
    private val timeRepository: ITimeRepository = mockk(relaxed = true)
    private val gameRepository: IGameRepository = mockk(relaxed = true)
    ...
    @Test
    fun `init success - sets mainState to Success`() = runTest {
    ...
    @Test
    fun `init failure - no internet - sets mainState to NoInternetConnection`() = runTest {
    ...
    @Test
    fun `when lastSinceStartDaysCount equals current, loads keyForms`() = runTest {
    ...
    @Test
    fun `when lastSinceStartDaysCount not equals current, resets keyForms and saves new day`() = runTest {
    ...
    @Test
    fun `when more than 1 day since last victory, resets victories row count`() = runTest {
    ...
    @Test
    fun `when 1 or less days since last victory, does not reset victories row count`() = runTest {
    ...
    fun setUp() {
        ...
        coEvery { configRepository.fetchAndActivateRemoteConfig() } just Runs
